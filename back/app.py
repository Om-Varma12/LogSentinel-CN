...
def build_incident_from_api(data, abs_index):
    ...
    parts = p["Request"].split()
    endpoint = parts[1] if len(parts) >= 2 else p["Request"]
...
# ─────────────────────────────────────────
import time
# At the bottom of the script, replace the JS block with:
if not st.session_state.get("done_notified"):
    time.sleep(2)
    st.rerun()