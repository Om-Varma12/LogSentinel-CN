...
export const GhostCursor: React.FC<GhostCursorProps> = ({
  ...
  function calculateScale() {
    // Use a fixed smaller scale for consistent cursor trail
    return 0.6;
  }
  ...
  useEffect(() => {
    ...
    // Attach renderer to document body for full viewport coverage
  ...
  return (
    <div ref={containerRef} className={`pointer-events-none fixed inset-0 ${className ?? ''}`} style={mergedStyle} />
  );
};
